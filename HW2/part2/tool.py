...
## TO DO ##
def plot_learning_curve(x, y, metric="Acc", modelname="Lenet"):
    """_summary_
    The function is mainly to show and save the learning curves. 
    input: 
        x: data of x axis 
        y: data of y axis (dict)
    output: None 
    """
    ### TO DO ### 
    plt.title(f'my{modelname} {metric}')
    plt.xlabel('Epoch')
    plt.ylabel(f'{metric}')
    plt.plot(x, y["Training"], label=f'Training {metric}')
    plt.plot(x, y["Val"], label=f'Validation {metric}')
    plt.legend()
    plt.savefig(f'my{modelname} {metric}.png')
    print(f'my{modelname} {metric}.png is saved!')
    plt.show()
    

def train(model, train_loader, val_loader, num_epoch, log_path, save_path, device, criterion, scheduler, optimizer, modelname):
    ...
    x = range(0,num_epoch)
    model_acc = {"Training": overall_acc.tolist() , "Val": overall_val_acc.tolist()}
    model_loss = {"Training": overall_loss.tolist() , "Val": overall_val_loss.tolist()}
    # Plot Learning Curve
    ## TO DO ##
    # Consider the function plot_learning_curve(x, y) above
    plot_learning_curve(x, model_acc, metric='Acc', modelname=modelname)
    plot_learning_curve(x, model_loss, metric='Loss', modelname=modelname)